...
std::vector<std::string> parseCsvLine(const std::string& line) {
    ...
    for (size_t i = 0; i < line.size(); i++) {
        ...
        if (inQuotes) {
            ...
            if (c == '"') inQuotes = true;
            else if (c == ',') { cells.push_back(current); current.clear(); }
            else current += c;
        }
    }
...
void writeBooks(const std::vector<Book>& books) {
    ...
    file << "id,title,author,year,status,genre,rating,notes,created_at,finished_at\n";
...
std::vector<Book> readBooks() {
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            b.finished_at = cells.size() > 9 ? cells[9] : "";
...
json bookToJson(const Book& b) {
    return json{
        ...
        {"status", b.status}, {"genre", b.genre}, {"rating", b.rating},
        {"notes", b.notes}, {"created_at", b.created_at}, {"finished_at", b.finished_at}
    };
}
...
int main() {
        ...
        try {
            ...
            for (auto& b : books) {
                if (b.id == id) {
                    ...
                    // finished_at cuma keisi SEKALI, pas pertama kali nyampe status selesai
    ...
    svr.Delete(R"(/books/(\d+))", [](const httplib::Request& req, httplib::Response& res) {
        int id = std::stoi(req.matches[1]);
        auto books = readBooks();
        auto it = std::remove_if(books.begin(), books.end(), [id](const Book& b) { return b.id == id; });
        if (it == books.end()) {
            res.status = 404;
            res.set_content(R"({"error":"Buku tidak ditemukan"})", "application/json");
            return;
